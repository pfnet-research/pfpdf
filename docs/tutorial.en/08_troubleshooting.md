...
`--doctor` does not download browsers, pull Docker images, create project or output directories, or change OS settings. For actual launch and mount checks it uses an isolated temporary profile or container, which is deleted after the check. Each external check times out after 10 seconds including cleanup, and the whole run after 60 seconds.
...
Working around it requires configuration changes with root privileges. On Ubuntu, for example, one known approach is adding an AppArmor profile for the Chromium executable. In environments where root access is impractical, consider using the Docker renderer.
...
- The default 5-minute timeout covers Docker image / browser preparation, readiness, rendering, and PDF post-processing / structural inspection combined. Check the debug log to see which phase consumed the time
...
- After a timeout, the existing PDF is not overwritten. There is a short grace period before child processes and containers are forcibly terminated, so wait for the CLI to exit before rerunning

## 8.9 Docker renderer errors

- Use `--doctor` to check that the Docker daemon is running
- With Docker Desktop, the directories containing the input and the logo must be included in file sharing. If they are not shared, the pre-run diagnostics report an error that includes the mount instructions
- The image is fetched at a pinned tag. To use a different version, specify `--docker-image`
- If a custom image's internal renderer protocol does not match the current pfpdf, specify an image of the matching version

## 8.10 The output file is not updated
