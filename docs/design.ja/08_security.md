...
- raw HTML と custom template は pfpdf process から参照可能な local file を読み、remote endpoint へ送信し得る。local / Docker renderer とも信頼できない入力の sandbox ではない
- 静的 local resource の `..`、absolute path、symlink target を input root だけの allowlist で制限しない
- script が実行時に生成する local path は静的 resource graph の対象外で、local / Docker renderer とも参照を保証しない
- ホストフォントの直接参照や read-only mount は再配布を避ける手段であり、利用権や PDF への埋め込み権を与えるものではない
...
- Docker renderer は input、template、logo、font の必要な directory を可能な限り read-only で mount する
...
- renderer phase 全体に finite な deadline を適用し、timeout 後は child / container と listener を回収する。trusted input であっても停止した image / browser download、script、browser、container を無期限に待たない

## 8.3 sandbox に関する位置付け

- pfpdf の local resource graph と Docker renderer の read-only mount は correctness と配布上の都合のための機構であり、信頼できない入力に対する security boundary ではない
...
- container 内でも Chromium sandbox を維持できる専用 seccomp / user namespace 構成を pinned browser で検証し、結果と判断根拠を `SECURITY.md` に記載する。`unconfined` や container 境界だけを根拠とする sandbox なし起動を release の既定にしない(05 章参照)
