...
- Replacement reason: Correct resolution of `..`, absolute paths, and nested CSS required a mapping table of static resources. Moreover, a simple server with wildcard CORS unnecessarily widens the scope of accidental file reads from other local web pages. Even without providing a sandbox, an exact-file map and a random token have value for correctness and defense in depth
...
## DD-06: Use a single rendering path

- Status: Accepted (2026-08)
- Problem: Multiple renderer backends and user-facing selection settings multiply execution paths, diagnostics, cleanup, and the test matrix, and can produce different behavior for the same input
- Options: (a) retain multiple backends with explicit switching, (b) limit pfpdf to a single Vivliostyle rendering path
- Adopted: (b). The rendering path is limited to the `AssetServer` and the Vivliostyle CLI child process, making configuration, diagnostics, and failure classification unambiguous
- Rejection reasons: (a) requires distribution, compatibility, timeout, and cleanup maintenance even for unused backends and spreads quality assurance across them
- Risk: When a browser cannot launch in a particular environment, there is no alternate backend to select. `--doctor`, an explicit browser path, and supported-environment testing help users diagnose and migrate
...
- Problem: Local assets referenced from Markdown, raw HTML, and nested CSS must be resolved with the correct base path while the renderer receives the same `document.html`
- Options: (a) serve the entire input directory or filesystem root, (b) embed all assets in the HTML as data URLs, (c) build a graph of static references in the parser and rewrite them to logical URLs
- Adopted: (c). Renderer-specific paths can be removed from the HTML, serving can be limited to required resources, and CSS import cycles are handled explicitly. The server uses a random path token and an exact-file map and starts on the same host loopback interface as the browser
- Rejection reasons: (a) invites overly broad serving to handle absolute paths, with remaining path collisions and platform differences. (b) increases memory usage for large fonts and images, and also conflicts with the policy of making single-file HTML a non-goal
- Risk: Static URL extraction from HTML and CSS becomes a new source of complexity. Local paths dynamically generated by scripts cannot be covered and are not guaranteed to resolve in the renderer. Assets are also not snapshotted during the build. Mitigated with dedicated parsers, a visited set, and manifest fixtures
- Verification: integration tests for nested `@import`, `url()`, `srcset`, absolute / `..` / symlink paths, cycles, logical URLs, and token / traversal / range handling. Also verify that the server delivers the same HTML byte sequence
...
- Verification: fault-injection tests for the prohibition of pagination before the readiness gate, readiness hangs, child hangs, source / copied byte-count mismatch, truncated trailer after copy, flush / rename failure, interruption, and preservation of existing output
...
- Verification: browser smoke tests including fonts and local SVG, a browser E2E test for registered promise rejection, and tests for resource MIME / load failure, notification failure, timeout, gate release, child abort, and preservation of existing output
...
- Pipeline: Applied as the final pass of the source section, after heading IDs, the table of contents, resource URLs, table decoration, MathJax, and syntax highlighting. It is included in the `document.html` consumed by the renderer, and no test-only pipeline is created
...
- Adopted: (e). Before the renderer starts, the SVG is inspected and normalized on the HAST and serialized to `generated/mermaid-NNNN.svg`. The renderer and tests consume the same generated assets, requiring no browser process or CDN beyond the existing renderer. The external SVG avoids the loss of inline SVG marker references in Vivliostyle's pagination DOM while keeping vectors, in-SVG text, and deterministic IDs
...
- Status: Accepted (2026-08)
- Problem: Markdown sources need a way to reference `.bib` files and generate inline citations, a deterministic bibliography, and internal PDF links without changing the existing GFM, HAST, and Vivliostyle pipeline, the handling of single- and multi-file input, or the shared HTML used by the renderer and tests
...
- Verification: unit tests for input, parser, formatter, and semantic HTML, preservation of existing output on bibliography errors, all-template PDFs / page images / links / `pdftotext` of the common preview, and renderer E2E tests
...
- Adopted: (c). Release Please maintains a release PR from Conventional Commits, while PR review decides the version and notes. A short-lived installation token from a release GitHub App scoped to this repository creates the PR and tag. The tag workflow builds the npm tarball and four PDFs exactly once, checks the same tarball on four environments, completes the PDF set on the draft Release, and publishes to npm via OIDC after approval in the `release` Environment. Manual resumes also pin the workflow ref to the existing release tag. The GitHub Release becomes public only after verifying the exact version from the public registry and rechecking the remote Release asset digests
...
- Scope: The official user-facing distribution is npm and the accompanying documents are delivered through GitHub Releases
...
- Verification: release-helper unit tests; tag / lock / CHANGELOG consistency checks; npm pack allowlist; packed-package install-lock / shrinkwrap equality, `npm ls`, and real PDF smoke tests on four environments; exact four-PDF set and checksums; draft asset state / size / remote SHA-256 digest checks; and an exact-version smoke test from the public registry
...
- Status: Accepted (2026-08)
- Problem: A document's intended appearance should be versioned together with its Markdown. Previewing, CI, and migration checks must still be able to apply another template without editing the source, and precedence relative to `PFPDF_TEMPLATE` / `--template` must be unambiguous
...
- Adopted: (c). `--template SOURCE` / `PFPDF_TEMPLATE` first checks for an exact bundled-preset name match, then classifies the value as a `git::` locator or local path. `--template-preset NAME` / `PFPDF_TEMPLATE_PRESET` explicitly requires a preset. `--logo SOURCE` / `PFPDF_LOGO` likewise accepts a Git locator or local path through one option. The repository path is checked as a directory or file according to its role, and the same URL / ref is shared within a build. With no explicit logo, the template slot's `src` is resolved as an ordinary resource; an explicit logo overrides it, and `--no-logo` removes it. This reuses the DOM slot and resource graph without a new manifest or template engine. Adding a repository template variant and four logo states raises effective-config schema to 4
- Rejected because: (a) has no package boundary that retrieves all three template files plus nested CSS / image / font assets at one revision. (b) adds discovery, signatures, version indexes, and distribution operations beyond current requirements. (d), dedicated options per source kind, multiplies partial combinations of CLI options and environment variables for each template and logo and cannot represent a source as one copyable, pinnable string
...
- Verification: Tests cover locator schemes / queries / traversal / credentials, commit checkout from a local Git fixture, nested template and logo paths, sharing of one repository, template default / explicit override / `--no-logo`, CLI / environment exclusivity, exit codes, and Japanese / English documentation builds
