...
- **template とロゴの注入** — 研究報告向け `academic`、長文向け `book`、短い資料向け `compact`、中立な `default`、カジュアルな小冊子向け `notebook`、企業文書向け `pfn`、高密度な技術文書向け `technical` の 7 つの template を同梱。front matter で選択でき、CLI から上書きできます。custom template は既定ロゴを持て、template と logo は local path または revision / サブディレクトリ付き Git repository locator で指定できます。bundled logo は含めません
